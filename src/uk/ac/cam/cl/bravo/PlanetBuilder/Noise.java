...
public class Noise {
	...
	public static double noise(double x, double y, double z, double w, int seed) {
		...
		double z0 = z - k + t;
		double w0 = w - l + t;
		int rankx = + fastbool(x0 > y0) + fastbool(x0 > z0) + fastbool(x0 > w0);
		int ranky = - fastbool(x0 > y0) + fastbool(y0 > z0) + fastbool(y0 > w0);
		int rankz = - fastbool(x0 > z0) - fastbool(y0 > z0) + fastbool(z0 > w0);
		int rankw = - fastbool(x0 > w0) - fastbool(y0 > w0) - fastbool(z0 > w0);
		int i1 = fastbool(rankx > 2 + 0);
		int j1 = fastbool(ranky > 2 + 1);
		int k1 = fastbool(rankz > 2 + 2);
		int l1 = fastbool(rankw > 2 + 3);
		int i2 = fastbool(rankx > 1 + 0);
		int j2 = fastbool(ranky > 1 + 1);
		int k2 = fastbool(rankz > 1 + 2);
		int l2 = fastbool(rankw > 1 + 3);
		int i3 = fastbool(rankx > 0 + 0);
		int j3 = fastbool(ranky > 0 + 1);
		int k3 = fastbool(rankz > 0 + 2);
		int l3 = fastbool(rankw > 0 + 3);
		double x1 = x0 - i1 + G;
		double y1 = y0 - j1 + G;
		double z1 = z0 - k1 + G;
		double w1 = w0 - l1 + G;
